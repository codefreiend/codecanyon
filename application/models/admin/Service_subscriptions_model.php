...
class Service_subscriptions_model extends CI_Model
{
    public $v_fields=array('services.service_name', 'userz.full_name');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->select("$table.*  , services.service_name as service_id  , userz.full_name as user_id ");
        $this->db->from($table);
         $this->db->join("services", "service_subscriptions.service_id = services.id", "left");  $this->db->join("userz", "service_subscriptions.user_id = userz.id", "left");     
