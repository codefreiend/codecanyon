...
class Ion_auth_model extends CI_Model
{
	...
	/**
	 * caching of userz and their groups
	 *
	 * @var array
	 **/
	 ...
	 * against an entry in the userz table.
	 ...
	 * Deactivate : Updates a userz row with an activation code.
	 *
	 * @author Mathew
	 */
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['userz'], $data, array('id' => $id));
		}
		else
		{
			$data = array(
	...
	public function clear_forgotten_password_code($code) {
		...
		if ($this->db->count_all_results($this->tables['userz']) > 0)
		{
			$data = array(
			...
			$this->db->update($this->tables['userz'], $data, array('forgotten_password_code' => $code));
	...
	public function reset_password($identity, $new) {
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['userz'], $data, array($this->identity_column => $identity));
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE)
		{
			...
			$successfully_changed_password_in_db = $this->db->update($this->tables['userz'], $data, array($this->identity_column => $identity));
	...
	public function username_check($username = '')
	{
		...
		return $this->db->where('username', $username)
										...
										->order_by("id", "ASC")
										->limit(1)
		                ->count_all_results($this->tables['userz']) > 0;
	}
	...
	public function forgotten_password($identity)
	{
		...
		$this->db->update($this->tables['userz'], $update, array($this->identity_column => $identity));
	...
	public function forgotten_password_complete($code, $salt=FALSE)
	{
		...
		$profile = $this->where('forgotten_password_code', $code)->userz()->row(); //pass the code to profile
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		// filter out any data passed that doesnt have a matching column in the userz table
		// and merge the set user data and the additional data
		$user_data = array_merge($this->_filter_data($this->tables['userz'], $additional_data), $data);

		$this->trigger_events('extra_set');

		$this->db->insert($this->tables['userz'], $user_data);

		$id = $this->db->insert_id($this->tables['userz'] . '_id_seq');
	...
	*/
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select(array(
			    $this->tables['userz'].'.*',
			    $this->tables['userz'].'.id as id',
			    $this->tables['userz'].'.id as user_id'
			));
		}
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$this->db->join(
				    ...
				    $this->tables['users_groups'].'.'.$this->join['userz'].'='.$this->tables['userz'].'.id',
				    'inner'
				);
			}
		...
		$this->response = $this->db->get($this->tables['userz']);
	...
	public function user($id = NULL)
	{
		...
		// if no id was passed use the current userz id
		$id = isset($id) ? $id : $this->session->userdata('user_id');

		$this->limit(1);
		$this->order_by($this->tables['userz'].'.id', 'desc');
		$this->where($this->tables['userz'].'.id', $id);
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
		                ->where($this->tables['users_groups'].'.'.$this->join['userz'], $id)
	...
	public function add_to_group($group_ids, $user_id=false)
	{
		...
		{
			if ($this->db->insert($this->tables['users_groups'], array( $this->join['groups'] => (float)$group_id, $this->join['userz'] => (float)$user_id)))
			{
	...
	public function remove_from_group($group_ids=false, $user_id=false)
	{
		...
		if( ! empty($group_ids))
		{
			...
			foreach($group_ids as $group_id)
			{
				$this->db->delete($this->tables['users_groups'], array($this->join['groups'] => (float)$group_id, $this->join['userz'] => (float)$user_id));
		...
		{
			if ($return = $this->db->delete($this->tables['users_groups'], array($this->join['userz'] => (float)$user_id))) {
	...
	public function update($id, array $data)
	{
		...
		$data = $this->_filter_data($this->tables['userz'], $data);
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['userz'], $data, array('id' => $user->id));
	...
	**/
	public function delete_user($id)
	{
		...
		// delete user from userz table should be placed after remove from group
		$this->db->delete($this->tables['userz'], array('id' => $id));
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['userz'], array('last_login' => time()), array('id' => $id));
	...
	public function remember_user($id)
	{
		...
		$this->db->update($this->tables['userz'], array('remember_code' => $salt), array('id' => $id));
	...
	public function login_remembered_user()
	{
		...
		if ($query->num_rows() == 1)
		{
			...
			// extend the userz cookies if the option is enabled
	...
	**/
	public function delete_group($group_id = FALSE)
	{
		...
		// remove all userz from this group
